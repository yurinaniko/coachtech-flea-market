...
Password：password
```
※ Seederでは10商品を作成していますが、
「自分が出品した商品は商品一覧・検索結果には表示されない」
という仕様を確認するため、ログイン中のテストユーザーが出品した商品は
商品一覧および検索結果には表示されません。

（マイページの「出品した商品」には、該当商品が表示されます）

そのため、商品一覧では9商品が表示されます。
...
会員登録画面およびログイン画面では、
FormRequest を使用してバリデーションを実装しています。
...
- 認証、商品操作、検索、お気に入り、購入処理、プロフィール更新などの主要機能について、
