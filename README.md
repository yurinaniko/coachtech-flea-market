...
※メール認証済み状態で作成されています。
このユーザーは商品・取引の紐付きはありません（住所プロフィールのみシーダーで登録済み）。
...
1. 出品者A（または出品者B）でログイン
   ※テストユーザーには取引データが無いため「取引中」タブは空です
2. マイページ → 「取引中」タブ
3. 商品を選択するとチャット画面が表示されます
...
※すべてのユーザーはメール認証済みです。
- テストユーザーは商品・取引の紐付きなし（住所プロフィールのみあり）。
- 出品者Aと出品者Bはそれぞれ5つずつ商品を出品。購入データは4件で、うち1件（HDD）は相互評価まで完了済みのため「取引中」タブには3件表示されます。
...
1. 出品者A（または出品者B）でログイン（Seederで購入済の取引あり）
2. マイページ → 「取引中」タブをクリック
3. 該当商品を選択するとチャット画面が表示される
...
phpMyAdmin： http://localhost:8082（DB確認用・MySQLはホスト側3308番で公開）
...
一方で、中間テーブルには重複登録を防ぐ複合ユニークキーを設定しています。
- favorites：`user_id` × `item_id`（同じ商品を二重にお気に入りできない）
- purchases：`user_id` × `item_id`（同じ商品を二重購入できない）
- category_item：`item_id` × `category_id`（同じカテゴリを二重付与できない）
...
- 住所変更画面
- Stripe決済
- 購入完了画面（カード・コンビニ決済とも決済後に表示）
- 取引チャット
- 取引完了 / 相互評価
- 評価依頼メール通知（MailHog）

※ 出品者A／出品者Bでログインすると、Seeder投入済みの取引でチャット・評価をすぐ確認できます。
...
- 商品一覧ページで入力した検索キーワードは、
  ログイン時はタブ（おすすめ / マイリスト）を切り替えても保持され、
  各タブ内で検索結果が反映されます。
  （未ログイン時はタブ切り替えでキーワードはリセットされます）
...
- 種類	バージョン
- PHP	8.3
- Laravel	12.x（8で構築後、8→9→10→11→12へ段階アップグレード済み。CIで composer audit --locked の厳格監査・脆弱性0件）
- Laravel Fortify / Sanctum（認証）
- stripe/stripe-php（決済SDK）
- MySQL	8.0
- Nginx	1.25
...
- 画像アップロード（storage 管理）
- 中間テーブルによる多対多管理
- 商品一覧画面でのおすすめ／マイリストの検索キーワード保持（ログイン時）
- phpMyAdmin（DB確認用）
...
- 出品者は下記のMailHog（http://localhost:8027）にアクセスすることでメールを確認できます
...
### 確認手順
1. 商品を購入し、マイページから取引中のタブを選択
2. 取引中のタブ内の商品を選択し、取引チャット画面を開く
3. 購入者が取引完了を行い、出品者を評価する
4. MailHog（http://localhost:8027）にアクセスする(実際では出品者のメールアドレスに届く)
5. 出品者宛の評価依頼メールを確認する
...
**全 90 テスト（Feature）が PASS することを確認しています。**
- CI（GitHub Actions）でもプッシュごとにテストと composer audit --locked による
依存脆弱性監査（厳格モード）を実行しています。
...
「画面で操作できないこと」と「サーバーが許可しないこと」は別物という前提に立ち、
**防御は必ずサーバー側に置く**方針で実装しています。主な設計判断は以下のとおりです
（各対策には再発防止のための自動テストを付けています）。

- **認可はサーバー側で再確認**：取引チャットの閲覧・投稿・削除、購入、プロフィール編集などは、
  URLのIDを受け取った側で「ログイン中の本人／取引の当事者か」をサーバーで判定します。
  画面上のボタン非表示は利便性のためであり、防御としては扱っていません。

- **二重売買の防止（多層防御）**：同一商品への同時購入で在庫整合性が崩れないよう、
  購入処理を**トランザクション＋行ロック**で囲み、さらに**データベースの一意制約**を
  最後の砦として置いています。アプリ側のチェックがすり抜けても、DB が重複を拒否します。

- **決済Webhookの検証方針**：外部サービスからのWebhookはCSRFトークンを持てないため、
  CSRF検証の対象外にしつつ、**署名検証で正当性を担保**しています
  （守りを「トークン」から「署名」へ移す設計）。

- **アップロード画像のファイル名をランダム化**：保存名を推測できない形にし、
  同時アップロード時の上書き事故と、URL推測による他者ファイルへの到達を防いでいます。

- **レート制限**：ログイン・登録・パスワードリセット・メール再送などの入口に、
  用途ごとに分離した**名前付きレート制限**を設定し、総当たりや大量リクエストを抑制します。

- **マスアサインメント対策**：所有者IDや価格などクライアントが書き換えてはいけない値は、
  リクエストをそのまま保存せず、サーバー側で明示的に設定しています。

> 本リポジトリは学習用のためローカル環境での動作確認を前提としています。
> 実運用時は、これらに加えて `APP_DEBUG=false` 等の本番設定、HTTPS強制、
> DB接続情報・管理ツールの非公開化などのインフラ側の対策が必要です。
